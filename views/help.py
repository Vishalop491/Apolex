...
class Dropdown(discord.ui.Select):
    def __init__(self, ctx, options):
        super().__init__(placeholder="Please select a page.", min_values=1, max_values=1, options=options,row=2)
        self.invoker = ctx.author
...
class View(discord.ui.View):
    ...
    def add_buttons(self):
        self.startB = Buttons(emoji="⏮️", style=discord.ButtonStyle.grey, command=self.set_page, args=0, ctx = self.ctx)
        self.backB = Buttons(emoji="◀️", style=discord.ButtonStyle.grey, command=self.to_page, args=-1, ctx = self.ctx)
        self.nextB = Buttons(emoji="▶️", style=discord.ButtonStyle.grey, command=self.to_page, args=+1, ctx = self.ctx)
        self.endB = Buttons(emoji="⏭️", style=discord.ButtonStyle.grey, command=self.set_page, args=len(self.options)-1, ctx = self.ctx)
        self.quitB = Buttons(emoji="⏹️", style=discord.ButtonStyle.grey, command=self.quit, ctx = self.ctx)
    ...
    def gen_embeds(self):
        options, embeds = [], []
        options.append(discord.SelectOption(label="Home", description="", emoji='<:top46:1015918758688202762>'))
        embeds.append(self.home)
        for cog in self.get_cogs():
            if "help_custom" in dir(cog):
                emoji, label, description = cog.help_custom()
                options.append(discord.SelectOption(label=label, description=description, emoji=emoji))
                embed = discord.Embed(description=f"`{cog.__doc__}`", color=0x2f3136)
                embed.set_author(name=self.ctx.message.author.name, icon_url=self.ctx.message.author.display_avatar.url)
                embed.set_footer(text="Total commands 186 | Total pages 15", icon_url="https://images-ext-2.discordapp.net/external/GdrUyTvsud0Y9ilDqHtgDDiRGca5GXWts28UKvl134E/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/992797622094016622/6105d9b9b7605e99557d318622e5f569.png")
