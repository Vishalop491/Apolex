...
class Role(commands.Cog):
    ...
    async def temp(self,ctx,role:discord.Role,time,*,user:discord.Member):
        ''' <:icons_text1:986851468097253417>`Temporarily give a role to any member` '''
        ...
        await ctx.send(f"Done | {role.name} added to user {user.mention}")
    ...
    async def remove(self,ctx,user:discord.Member,role:discord.Role):
        ''' <:icons_text1:986851468097253417>`Remove a role from any member` '''
        if role == ctx.author.top_role:
                return await ctx.send("That role has the same position as your top role!")
        else:
          if role.position >= ctx.guild.me.top_role.position:
                return await ctx.send(f"This role is higher than my role, move it to the top!")
        await user.remove_roles(role)
        await ctx.send(f"Done | Role {role.name} removed from {user.mention}")
    ...
    async def delete(self,ctx,role:discord.Role):
        ''' <:icons_text1:986851468097253417>`Deletes the role from server` '''
        if role == ctx.author.top_role:
                return await ctx.send("That role has the same position as your top role!")
        else:
          if role.position >= ctx.guild.me.top_role.position:
        ...
        await role.delete()
        await ctx.send(f"Done, role {role} deleted")
    ...
    async def create(self,ctx,*,name):
        ''' <:icons_text1:986851468097253417>`Creates a role in the server` '''
        await ctx.guild.create_role(name=name,color=discord.Color.default())
        await ctx.send(f"Done | Role {name} created successfully!")
    ...
    async def rename(self,ctx,role:discord.Role,*,newname):
        ''' <:icons_text1:986851468097253417>`Renames any role` '''
        # BUG - The role with multiple names can't be changed with this
        # prev_name = role.name
        if role is None:
            return await ctx.send(f" | No role named {role} found in this server.")
        await role.edit(name=newname)
        await ctx.send(f" | Role {role.name} has been renamed to {newname}")

    @role.command()
    @commands.has_permissions(manage_roles=True)
    @commands.bot_has_permissions(manage_roles=True)
    async def color(self,ctx,role:discord.Role,color):
        ''' <:icons_text1:986851468097253417>`Changes the color of any role` '''
        if role is None:
            return await ctx.send(f" | No role named {role} found in this server.")
        ...
        await ctx.send(f" | Role {role.name}'s color has been changed to {color}")

    @role.command(aliases=["list-in-role"])
    # @commands.bot_has_permissions(manage_roles=True)
    # @commands.has_permissions(manage_roles=True)
    async def list(self,ctx,*,role:discord.Role):
        ''' <:icons_text1:986851468097253417>`Gives the list of members having specified role` '''
        role_name = role.name
        if role is None:
        ...
        if len(role.members) != 0:
            ...
            if nums <= 25:
                ...
                for l in counts:
                    ...
                    # print(first_num)
                    # print(last_num)
                    em = discord.Embed(description=" ",color=0x2f3136).add_field(name=f"Members with role {role} - [{len(emoji_list)}]",value="\n".join(f"`[{count+1}]` {emoji_list[count]} | {member_list[count]}" for count in range(first_num,last_num)))
                    embeds.append(em)
