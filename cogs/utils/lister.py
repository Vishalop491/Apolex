...
async def lister(ctx, your_list, color, *, title):
    ...
    for l in counts:
        first_num = l[0]
        last_num = l[1]
        em = discord.Embed(description=" ", color=color).add_field(
            name=f"{title} - [{len(your_list)}]",
            value="\n".join(f"`[{count+1}]` {your_list[count].mention}"
                            for count in range(first_num, last_num)))
        embeds.append(em)
...
async def lister_str(ctx, your_list, color, *, title) -> None:
    ...
    for l in counts:
        ...
        em = discord.Embed(description=" ", color=color).add_field(
            name=f"{title} - [{len(your_list)}]",
            value="\n".join(f"`[{count + 1}]` {your_list[count]}"
                            for count in range(first_num, last_num)))
        embeds.append(em)
