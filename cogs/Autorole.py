...
# Schema: {"guild": INT, "bots": LIST, "humans": LIST, "autorole": []}
    ...
    async def humans(self, ctx, *, r: discord.Role):
        if data := await self.db.find_one(
            ...
            await ctx.send(f"<a:nr_tick:956867762398044200>  | Autorole Humans Updated to `{r.name}`")
    ...
    async def add(self, ctx, r: discord.Role):
        ...
        if data := await self.db.find_one(
            ...
            await ctx.send(f"<a:nr_tick:956867762398044200>  | Autorole Updated to `{r.name}`")
    ...
    async def bots(self, ctx, *, r: discord.Role):
        if data := await self.db.find_one(
            ...
            await ctx.send(f"<a:nr_tick:956867762398044200>  | Autorole Bots Updated to `{r.name}`")
