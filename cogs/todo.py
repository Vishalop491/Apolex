...
class Todo(commands.Cog):
    ...
    async def todo(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            if not self.todo_list:
                ...
                embed = discord.Embed(title="{}'s Todo list:".format(ctx.message.author.name), description="", color=0x2f3136)
                ...
                for entry in sorted_keys:
                    ...
                    if len(embed.description + description) > 2000:
                        ...
                        embed = discord.Embed(title="{}'s Todo list:", color=0x2f3136.format(ctx.message.author.name), description="")
                        embed.description = description

                all_entries.append(embed)
                for count, embed in enumerate(all_entries):
                    if len(all_entries) > 1:
                        embed.title = "{}'s Todo list ({}/{}):".format(ctx.message.author.name.format(), count+1, len(all_entries), color=0x2f3136)
    ...
    async def add(self, ctx, *, msg):
      ...
      #  await ctx.message.delete()
        ...
        self.save_list()
        await ctx.send("Successfully added `{}` to your todo list!".format(msg))
    ...
    #    await ctx.message.delete()
        if not self.todo_list:
            ...
            if found:
                self.save_list()
                await ctx.send("Successfully removed `{}` from your todo list!".format(msg))
            else:
    ...
    async def clear(self, ctx):
      ...
      #  await ctx.message.delete()
        ...
        self.save_list()
        await ctx.send("Successfully cleared your todo list!")
                    ...
                    if self.todo_list[entry][0] < current_time():
                        ...
                        if self.todo_list[entry][3] is True:
                            if self.bot.notify['type'] == 'msg':
                                ...
                                em = discord.Embed(title='Timer Alert', color=0x2f3136,
