...
class logging(commands.Cog):
    ...
    async def channel(self, ctx, c: discord.TextChannel):
        try:
            ...
            await ctx.send(f"<a:nr_tick:956867762398044200>  | All logs channel are updated to <#{c.id}>")
    ...
    async def config(self, ctx):
        ...
        x = data["log-channel"]
        if x == None:
          embed = discord.Embed(title=f"Logging channel:", description=f"No Logging Channel Found", color = 0x2f3136)
          await ctx.send(embed=embed)
        else:
          embed = discord.Embed(title=f"Logging channel:", description=f"<#{x}>", color = 0x2f3136)
          await ctx.send(embed=embed)
    ...
    async def delete(self, ctx: commands.Context):
        ...
        await ctx.send(f'<a:nr_tick:956867762398044200> | Successfully Deleted Logging Channel')
    ...
    async def on_guild_emoji_create(self, emoji):
        with open("./data/databases/logging.json") as f:
            ...
            channel = self.bot.get_channel(x)
            embed = discord.Embed(description=f"Emoji ({emoji}) has been added.", color = 000000)
