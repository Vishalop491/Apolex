...
class invites(commands.Cog):
    ...
    async def invites(self, ctx, member: discord.Member=None):
        ...
        embed = discord.Embed(title = f'Invites For {member.name}#{member.discriminator}', description=f'{member.mention} Currently Has **{total}** Invites (**{normal}** Normal, **{left}** Leaves, **{fake}** Fakes)', timestamp = datetime.now(), color = 0x2f3136)
    ...
    async def resetall(self, ctx):
        ...
        embed = discord.Embed(title=f'Apolex invite tracker', description=f'I Have Successfully Reset The Guild\'s Invites!', timestamp = datetime.now(), color = 0x2f3136)
        embed.set_footer(text = f'Requested By {ctx.author.name}')
    ...
    async def leaderboard_invites(self, ctx):
        ...
        embed = discord.Embed(title=f"Leaderboard Page {current}", description="", colour=0x2f3136)
